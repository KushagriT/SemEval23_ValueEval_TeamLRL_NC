...
model_aux = RobertaForSequenceClassification.from_pretrained('/valueeval_model_level1')
model_aux.to(device)
...
model_level1 = RobertaForSequenceClassification.from_pretrained('/valueeval_model_level1')
...
class modellabel(RobertaForSequenceClassification):
  def __init__(self, config, model1, model2):
        ...
        self.modelaux = model2
        self.out_proj = nn.Linear(config.hidden_size + self.modellevel1.config.hidden_size+ self.modelaux.config.hidden_size, config.num_labels)
  ...
  def forward(self,input_ids = None,attention_mask = None,labels = None, return_dict= None):
    ...
    seq_aux = self.modelaux.classifier.dense(self.modelaux.roberta(input_ids,
            attention_mask=attention_mask,
            return_dict=return_dict).last_hidden_state[:,0,:])
    
    features = torch.cat([sequence_output,seq_level1,seq_aux],1)
...
model_main = modellabel.from_pretrained('/roberta_valueval_pretrain',
                                                                num_labels=NUM_LABELS_LABELS,
                                                                problem_type="multi_label_classification",
                                    model1 = model_level1,
                                    model2 = model_aux,
                                    )
model_main.to(device)
for param in model_main.roberta.parameters():
      param.requires_grad=False
modules = [model_main.roberta.encoder.layer[10], model_main.roberta.encoder.layer[11]] 
for module in modules:
  for param in module.parameters():
    param.requires_grad = True
...
args = TrainingArguments(
        ...
        num_train_epochs=epochs_labels,
        weight_decay=0.01,
...
multi_trainer.train()
multi_trainer.save_model('/system1')

df_val = pd.read_csv('/arguments-test.tsv',sep='\t')
df_val['Stance'] = df_val['Stance'].replace({'in favour of':'in favor of'})

df_val_comp = pd.DataFrame()

df_val_comp ['text'] = df_val ['Conclusion'] + ' </s> ' + df_val['Stance'] + ' </s> ' + df_val['Premise']
df_val_comp ['Argument ID'] = df_val['Argument ID']
sort_ind_val = df_val_comp.text.str.len().sort_values().index
df_val_comp = df_val_comp.reindex(sort_ind_val)

df_val_comp.reset_index(inplace=True)

dtst_val_ = Dataset.from_pandas(df_val_comp)
...
val_label_dataset = dtst_val_.map(preprocess_data, remove_columns=dtst_val_.column_names,batched=True,batch_size=BATCH_SIZE)
preds = multi_trainer.predict(val_label_dataset)
...
preds_df['Argument ID'] = val_label_dataset['Argument ID']
cols = list(preds_df.columns)
cols = [cols[-1]] + cols[:-1]
preds_df = preds_df[cols]
preds_df.to_csv('/predictions_arg_test_system1.tsv',sep='\t',index=False)